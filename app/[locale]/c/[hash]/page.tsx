...
export async function generateMetadata({
  params,
  searchParams,
}: SmsPreviewHashPageProps): Promise<Metadata> {
  ...
  const title = "Confirm Your Pharmacy | ClearRx";
  const description = "Securely verifying your prescription information";
...
export default async function SmsPreviewHashPage({
  params,
  searchParams,
}: SmsPreviewHashPageProps) {
  ...
  return (
    <div style={{ padding: 32, fontFamily: "monospace" }}>
      <p>
        <strong>locale:</strong> {imageLocale}
      </p>
      <p>
        <strong>hash:</strong> {hash}
      </p>
      <p>
        <strong>type:</strong> {type ?? "(none)"}
      </p>
      <p>
        <strong>image:</strong> {imageName}
      </p>
      <p>
        <strong>og:image:</strong> https://clearrx.co/sms-preview/{imageLocale}/
        {imageName}
      </p>
    </div>
  );
